...
\newcommand{\fromtabs}[2]{\textit{Tables \ref{tab:#1} and \ref{tab:#2}}}% START OF THEORY
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption*{Unsyncronized $\textbf{e}^-$ gun period, $T_g = 5ns$}
    \end{subfigure}
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{./figures/rhodoSim/9_3ns_gnuplot.png}
        \caption*{Syncronized $\textbf{e}^-$ gun period, $T_g = 9.3ns$}
    \end{subfigure}
...
An issue regarding the $\textbf{e}^-$ - $\vecthreeBF{B}$ interaction became apparent when energy gain during these interactions was observed.
A setup simulation was implemented in which a bunch of 100$\textbf{e}^-$ at $1$MeV was fired to a uniform magnetic field of $0.1$T placed in $x>0.05$m.
Initial results at $dt=0.01$ns proved the suspicion of $\textbf{e}^-$ - $\vecthreeBF{B}$ interaction being broken. However, the energy gain would decrease tremendously as $dt$ decreased.
...
Until this point, \textit{Rhodotron Simulation} have been using \fromsec{leapfrog} for $\textbf{e}^-$ - \textbf{EM} interactions. 
To test newer approaches, two additional version of  $\textbf{e}^-$ - \textbf{EM} interaction that are using \fromsec{rungekutta} were added.
\subsubsection{RK4-1}
First approach for integrating \fromsec{rungekutta} into $\textbf{e}^-$ - \textbf{EM} interaction was to calculate $\vecthreeBF{a}_{E}$ and $\vecthreeBF{a}_{B}$ from \fromeq{acc_of_E_and_B} using \textit{RK4}.
After $\vecthreeBF{a}_{EM} = \vecthreeBF{a}_{E} + \vecthreeBF{a}_{B}$ was calculated, $\textbf{e}^-$ would move and accelerate using the \textit{Leap-frog} method. 
The idea was to produce more refined interaction results, leading to improved accuracy especially in $\textbf{e}^-$ - $\vecthreeBF{B}$.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Comparing Leap-frog, RK4-1, RK4-2 performance on $\textbf{e}^-$ - $\vecthreeBF{B}$ interaction\\ $E_{in}=1$MeV, \textbf{B}=$0.1$T, $t_{end}=5$ns}
...
\begin{figure}[H]
    ...
    \end{minted}
\caption{Logic of is $\textbf{e}^-$ inside the shape of magnet}
...
\begin{figure}[H]
    ...
    \end{minted}
\caption{RK4-1 implemenation of $\textbf{e}^-$-$\vecthreeBF{B}$}
\label{fig:rk1_B}
\end{figure}
\begin{figure}[H]
    \centering
    ...
    \end{minted}
\caption{RK4-1 implemenation of $\textbf{e}^-$-\textbf{EM}}
\label{fig:rk1_EM}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{minted}
\caption{RK4-2 implemenation of $\textbf{e}^-$-\textbf{EM}}
\label{fig:rk2_EM}
\end{figure}

...
% TODO : ADD SUPERFISH AND CST SIMULATIONS OF A SIMPLE RHODOTRON CAVITY MAYBE?????
...
\begin{description}
    \item[First acceleration] Electrons in the rhodotron cavity are accelerated by the electric field created between two coaxial cylinders, towards the inner cylinder when they are ejected into the cavity. See \fromfig{rhod_cycle_1}
    \item[Inner shielding] While inside the inner cylinder, the cylinder acts as a faraday cage and shields the electrons inside while the electric field is being reversed.See \fromfig{rhod_cycle_2}
    \item[Second acceleration] Once the electrons leave the inner cylinder, they accelerate towards the outer cylinder by the reversed electric field until they leave the cavity. See \fromfig{rhod_cycle_3}
    \item[Magnets] After leaving the cavity, an electromagnet placed in their path steers the electrons back into the cavity in which time the electric field changes the direction again. See \fromfig{rhod_cycle_4}
\end{description}
...
Simulation frame after a simulation run has been completed can be observed in \fromfig{gui_simulate_1}.
\begin{figure}
    \centering
...
Two example renders can be seen in \fromfigs{gui_render_1}{gui_render_2}
\begin{figure}
    \centering
...
In \fromfig{gui_analyze_Edist}, \textit{Analyze Frame} plotting \textbf{E} distribution histogram, and in \fromfigs{gui_analyze_Et}{gui_analyze_Et2}, \textit{Analyze Frame} plotting \textbf(E)(t) graphs can be observed.
\begin{figure}
    \centering
...
In the following \fromfigf{gui_sweep_running}{gui_sweep_muE}{gui_sweep_sE}{gui_sweep_sR}, this frame can be inspected in detail.
\begin{figure}
    \centering

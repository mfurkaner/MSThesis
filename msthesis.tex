...
\end{eqnarray}
$\gamma^2 \beta^2 + 1 = \gamma^2 $ and \fromeq{a_F_parallels} can be used to achieve
\begin{eqnarray}
...
\begin{figure}[H]
    \begin{eqnarray}
        ...
        E_{POC} &=& 0.532 MeV \nonumber
    \end{eqnarray}
    \caption*{$P_{diss}=40$kW, $R_1=0.188$m, $R_2=0.753$m, $f=107.5$MHz}
\end{figure}
...
However, we cannot make the same claim regarding \fromeq{W_gain_each_pass_pottier}.
This inconsistency between $E_{Pottier}$ and $E_{CST}$, $\Delta E_{(Pottier-CST)}/E_{CST} \approx 14\%$ was noticed in the earlier simulations as well, which supported the idea of using another simulation software to reduce reliance on \textit{CST}.

...
\examineri{Assist. Prof. Bora Akgün}
...
\begin{abbreviations}
   ...
   \sym{Hz}{Hertz}
   \sym{KAHVELab}{Kandilli Detector Accelerator and \\Instrumentation Laboratory \vspace{5pt}}
   \sym{LF}{Leap-Frog}
...
\newcommand{\fromeq}[1]{Equation (\ref{eq:#1})}
\newcommand{\fromeqs}[2]{Equations (\ref{eq:#1} and \ref{eq:#2})}
\newcommand{\fromeqsth}[3]{Equations (\ref{eq:#1}, \ref{eq:#2} and \ref{eq:#3})}
\newcommand{\fromeqsf}[4]{Equations (\ref{eq:#1}, \ref{eq:#2}, \ref{eq:#3} and \ref{eq:#4})}

\newcommand{\fromfig}[1]{Figure \ref{fig:#1}}
\newcommand{\fromfigs}[2]{Figures \ref{fig:#1} and \ref{fig:#2}}
\newcommand{\fromfigth}[3]{Figures \ref{fig:#1}, \ref{fig:#2} and \ref{fig:#3}}
\newcommand{\fromfigf}[4]{Figures \ref{fig:#1}, \ref{fig:#2}, \ref{fig:#3} and \ref{fig:#4}}
\newcommand{\fromfigsix}[6]{Figures \ref{fig:#1}, \ref{fig:#2}, \ref{fig:#3}, \ref{fig:#4}, \ref{fig:#5} and \ref{fig:#6}}
...
The coefficients of any Runge-Kutta method can be visualized by a tableau called Butcher Tableau which can be observed in \fromfig{Butcher}.
\begin{figure}[h!]
    \[ 
    \begin{array} 
        {c|cccc}
...
\vspace{-13pt}
The simplest Runge-Kutta method is the Euler's method the Butcher tableau of which can be observed in \fromfig{Butcher_euler}.
...
Two of the most used Butcher tableaus for RK4 given in \fromfig{Butcher-RK4}.
The second tableau in \fromfig{Butcher-RK4} is called the ``3/8 rule''. Its main advantage is that its error coefficients are smaller than the other. But it costs more floating point operations per step. 
...
\begin{figure}[H]
    \centering
    \subfigure{{\includegraphics[width=.45\textwidth]{./figures/design/Ltot.png} }}%
    \qquad\subfigure{{\includegraphics[width=.45\textwidth]{./figures/design/tt300_mag.png} }}%
    \vspace{0pt}
...
\vspace{-10pt}
\fromeqs{magnet_rho_constraint}{magnet_L_constraint} define 2 constraints. As already discussed, it has been assumed that cavity design is set (\textit{i.e} $R_2$, $\lambda$, $d$ are already defined).
...
\iffalse \begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Cross sections of initial design.}
...
\begin{figure}[H]
    \centering
    \subfigure{{\includegraphics[width=.45\textwidth]{./figures/cst/cst_first_magnet_design1.png} }}%
    \qquad\subfigure{{\includegraphics[width=.46\textwidth]{./figures/cst/cst_first_magnet_design2.png} }}%
    \vspace{0pt}
...
After adding two more magnets and using $\gamma=15^\circ$, the second prototype design was created (\fromfig{initial_three_magnet_design}).
...
\begin{figure}[H]
    \centering
    \subfigure{{\includegraphics[width=.45\textwidth]{./figures/cst/cst_sinan_cavity_design1.png} }}%
    \qquad\subfigure{{\includegraphics[width=.47\textwidth]{./figures/cst/cst_sinan_cavity_design2.png} }}%
    \vspace{0pt}
...
After simulating the synchronous electron mentioned in \fromsec{cavity_of_a_rhodotron} 
with $\phi_{lag}=15^\circ$, $P_{diss}=40$kW, $R_1=0.188$m, $R_2=0.753$m, $f=107.5$MHz for one pass, 
the results from POC, CST and \fromeq{W_gain_each_pass_pottier} were found to be
% 40 kW total power
...
\begin{figure}[H]
    \begin{minted}[linenos=false, autogobble, frame=lines, framesep=2mm, fontsize=\footnotesize]{c++}
...
\vspace{-15pt}
As can be observed in \fromeq{poc_E_results}, POC software and CST produced similar results; $\Delta E_{(POC-CST)}/E_{CST} \approx 2\%$.
while \fromeq{W_gain_each_pass_pottier} was noticably different; $\Delta E_{(Pottier-CST)}/E_{CST} \approx 14\%$.
This inconsistency between \fromeq{W_gain_each_pass_pottier} and  CST simulation was noticed in the earlier simulations as well, which supported the idea of using another simulation software to reduce reliance on CST.
...
Initial step of improving POC towards Rhodotron Simulation was to implement $L_{out}$ optimizations to help optimazing magnet designs, as discussed in \fromsec{parameter_sweep}.
...
$E_{rms}$ and $R_{rms}$ would be vaguely dependent of each other; therefore, initial implementation of $\phi_{lag}$ sweep was based on minimizing $E_{rms}$ during simulation (see \fromfig{phlag_opt}). 
...
\begin{figure}[H]
    \centering
    %\captionsetup{justification=centering}
    \begin{minted}[linenos=false, autogobble, frame=lines, framesep=2mm, fontsize=\scriptsize]{c++}
...
\vspace{-15pt}
Since $\phi_{lag}$ is relatively easy to change after production, another version of \fromfig{phlag_opt} that was modified for given magnet design parameters was also implemented ({see \fromfig{phlag_opt_n_pass} in \fromapp{intermediate_codes}). 
...
The refactoring effort included proper implementation of OOP, details of which can be seen in \fromfigsix{e_class}{bunch_class}{gun_class}{sim_class}{rhodosim_class}{rf_class} of \fromapp{intermediate_codes}.
...
Implementations for \eE and \eB interactions in 3D can be found in \fromfigs{3D_e_E_interaction_first}{3D_e_B_interaction_first} of \fromapp{intermediate_codes}, 
...
\begin{figure}
    \centering
    \subfigure[\centering $dt=10^{-2}$ns, $\Delta E=2.783$MeV]{{\includegraphics[width=0.9\textwidth]{./figures/rhodoSim/mag_lf_001dt.png} }}%
    \vspace{1pt}
    \subfigure[\centering $dt=10^{-4}$ns, $\Delta E=0.011$MeV]{{\includegraphics[width=0.9\textwidth]{./figures/rhodoSim/mag_lf_00001dt.png} }}%
    \vspace{5pt}
    \caption{Energy gain of $1$MeV bunch in \textbf{B}=$0.1$T.}
    \label{fig:mag_lf_render}
\end{figure}
...
Until this point, Rhodotron Simulation have been using \fromsec{leapfrog} for \eEM interactions. 
...
First approach for integrating \fromsec{rungekutta} into \eEM interaction was to calculate $\vecthreeBF{a}_{E}$ and $\vecthreeBF{a}_{B}$ from \fromeq{acc_of_E_and_B} using RK4.
...
\begin{figure}[h]
    \centering
    \vspace{-5pt}
    \subfigure[\centering $dt$ vs $\Delta E$]{{\includegraphics[width=0.9\textwidth]{./figures/analiz/mag_lf_rk2_dt-E.png} }}%
    \vspace{1pt}
    \subfigure[\centering $dt$ vs $T_{sim}$]{{\includegraphics[width=0.9\textwidth]{./figures/analiz/mag_lf_rk2_dt-Tsim.png} }}%
    \vspace{5pt}
    \caption{Comparing Leap-frog, RK4-2 performance on \eB interaction $E_{in}=1$MeV, \textbf{B}=$0.1$T, $t_{end}=5$ns.}
    \label{fig:mag_lf_rk2_comparison}
\end{figure}
...
RK4-2 renders from these test can be found in \fromfig{mag_rk_render} of \fromapp{example_simulation_runs}.
...
\begin{figure}[H]
    \centering
    \subfigure[\centering Setup-1 parallel $\vecthreeBF{E}$ field]{{\includegraphics[width=0.381\textwidth]{./figures/illustrations/statE_1.png} }}%
    \qquad\subfigure[\centering Setup-2, perpendicular $\vecthreeBF{E}$ field]{{\includegraphics[width=0.389\textwidth]{./figures/illustrations/statE_2.png} }}%
    \vspace{5pt}
    \caption{Illustration of test setups.}
\end{figure}
\clearpage
...
Simulation frame while a simulation is running can be observed in \fromfig{gui_simulate_1}.
...
\begin{figure}[!th]
    \centering
    \includegraphics[angle=90,origin=c, width=.95\linewidth]{./figures/teknikcizim/Rhodotron Elektron Hizlandirici Sistemi.pdf}
    \vspace{10pt}
...
\begin{figure}[H]
    \centering
    \subfigure[\centering Bottom part of the cavity while polishing]{{\includegraphics[width=.46\textwidth]{./figures/manif/polish/rhodo_bottom_unpolished_cropped.jpeg} }}%
    \qquad\subfigure[\centering Top part of the cavity after polishing]{{\includegraphics[width=.46\textwidth]{./figures/manif/polish/rhodo_upper_polished_cropped.jpeg} }}%
    \vspace{5pt}
    \caption{Polishing of the inner surface of cavity.}
    \label{fig:manif_polishing}
    \vspace{5pt}
...
\begin{figure}[H]
    \centering
    \subfigure[\centering Bottom part of the cavity with the exposed beam line in inner cylinder]{{\includegraphics[width=.46\textwidth]{./figures/manif/toroidal_sheets/rhodo_bottom_cropped.jpeg} }}%
    \qquad\subfigure[\centering Bottom part of the cavity with the beam line assembled]{{\includegraphics[width=.46\textwidth]{./figures/manif/toroidal_sheets/rhodo_middle_toroidal_sheets_welded_cropped.jpeg} }}%
    \vspace{5pt}
    \caption{Deformation prevention measure, 10 mm thick toroidal sheets.}
    \label{fig:manif_toroidal_sheets}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subfigure[\centering Welding sheet bars]{{\includegraphics[width=.46\textwidth]{./figures/manif/welding/rhodo_assembled_welding_cropped.jpeg} }}%
    \qquad\subfigure[\centering Welding toroidal sheets]{{\includegraphics[width=.46\textwidth]{./figures/manif/welding/rhodo_middle_welding_toroidal_sheets_cropped.jpeg} }}%
    \vspace{5pt}
...
\begin{figure}[H]
    \centering
    \subfigure[\centering Beam enter \& exit flanges]{{\includegraphics[width=.46\textwidth]{./figures/manif/assembled/rhodo_final_cropped_5.jpeg} }}%
    \qquad\subfigure[\centering RF input \& probe flanges]{{\includegraphics[width=.46\textwidth]{./figures/manif/assembled/rhodo_final_4_cropped.jpeg} }}%
    \vspace{5pt}
...
\begin{figure}[H]
    \centering
    %\captionsetup{justification=centering}
    \begin{minted}[linenos=false, autogobble, frame=lines, framesep=2mm, fontsize=\tiny]{c++}
...
\begin{figure}[H]
    \centering
    %\captionsetup{justification=centering}
    \begin{minted}[linenos=false, autogobble, frame=lines, framesep=2mm, fontsize=\scriptsize, style=staroffice]{c++}
...
\begin{figure}
    \centering
    \subfigure[\centering $dt=10^{-2}$ns, $\Delta E=1.047$MeV]{{\includegraphics[width=0.85\textwidth]{./figures/rhodoSim/mag_rk_001dt.png} }}%
    \vspace{10pt}
    \subfigure[\centering $dt=10^{-4}$ns, $\Delta E=0.006$MeV]{{\includegraphics[width=0.85\textwidth]{./figures/rhodoSim/mag_rk_00001dt.png} }}%
...
\begin{figure}[H]
    \centering
    %\captionsetup{justification=centering}
    \begin{minted}[linenos=false, autogobble, frame=lines, framesep=2mm, fontsize=\tiny, style=staroffice]{c++}
...
\begin{figure}[H]
    \centering
    \subfigure{{\includegraphics[width=0.8\linewidth]{./figures/analiz/lf_rk1_rk2_dt-E_3.png} }}%
    \vspace{10pt}
    \subfigure{{\includegraphics[width=0.8\linewidth]{./figures/analiz/lf_rk1_rk2_dt-Tsim_3.png} }}%
...
\begin{figure}[H]
    \centering
    \subfigure{{\includegraphics[width=0.9\linewidth]{./figures/analiz/staticE_lf_rk2_dt-E.png} }}%
    \vspace{10pt}
    \subfigure{{\includegraphics[width=0.9\linewidth]{./figures/analiz/staticE_lf_rk2_dt-Tsim.png}}}%
